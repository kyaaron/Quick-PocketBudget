...
## Optimizations
- (High) I need Total Balance to show Monthly income value - Monthly expenses
- (High) I need to += values to the table when they are entered
- (High) I need to add or subtrack from localStorage.getItem("amount") as income and expenses are entered
...
For JavaScript, I learned how you can use `JSON.stringify` to store objects as values in localHost, making it possible to loop through these objects and tally up values to put on the screen. This resembles something more like a database.
